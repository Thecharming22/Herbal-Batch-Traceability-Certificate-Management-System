...
export default function Dashboard() {
  ...
  const fetchDashboard = async () => {
    ...
    await new Promise((resolve) => setTimeout(resolve, 3000));
      ...
      const dispatched = data.filter((b) => b.status === "Dispatched").length;
      const certificates = data.filter((b) => b.certificateFileName).length;
      const totalYield = data.reduce((sum, b) => sum + Number(b.yield), 0);

      const averageYield =
        totalBatches > 0 ? (totalYield / totalBatches).toFixed(2) : 0;
      ...
      const predictedYield =
        totalBatches > 0 ? Math.round(Number(averageYield)) : 0;
  ...
  const filteredBatches = dashboardData.recentBatches.filter(
    (batch) =>
    ...
    ☰
                        ...
                        {notifications.length === 0 ? (
                          ...
                          notifications.slice(0, 5).map((item) => {
                            ...
                            return (
                              <div
                                key={item._id}
                                className={`${bg} border-l-4 ${border} rounded-xl px-4 py-3 flex gap-3 shadow-sm hover:shadow-md transition-all duration-200 ${
                              ...
                              >
                                ...
                                <div className="flex-1">
                                  ...
                                  <p
                                    className="text-sm text-gray-600 leading-5 mt-1"
                                    style={{ fontFamily: "Times New Roman" }}
                                  >
                                    {item.message}
                                  </p>
                    ...
                    <div className="absolute right-0 mt-3 w-72 bg-black rounded-xl shadow-xl border border-gray-700 z-50 overflow-hidden">
                      ...
                      <div className="p-2 space-y-2 bg-black">
                        <Link
                          ...
                          className="flex items-center gap-3 px-3 py-3 rounded-lg bg-green-900/40 hover:bg-green-800/60 transition"
                        >
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-3 py-3 rounded-lg bg-red-900/40 hover:bg-red-800/60 transition"
                        >
                          <span className="text-lg">🚪</span>
       ...
       <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 lg:gap-6 mb-12">
            ...
            <div className="bg-green-950 backdrop-blur-sm text-white shadow-lg p-6 rounded-lg mt-8">
              <h2 className="text-lg md:text-2xl font-extrabold bg-gradient-to-r from-yellow-400 via-yellow-200 to-yellow-500 bg-clip-text text-transparent animate-shine">
                AI Insights
              </h2>

              <div className="border-t border-gray-400 pt-4 mt-2">
                <h3 className="text-lg md:text-2xl font-extrabold bg-gradient-to-r from-yellow-400 via-yellow-200 to-yellow-500 bg-clip-text text-transparent animate-shine">
                  Average Yield
                </h3>
                <p className="text-base md:text-2xl mt-2">
                  {dashboardData.averageYield} ml
                </p>
              </div>
              ...
              <div className="border-t border-gray-400 pt-6 mt-6">
                ...
                <div className="overflow-x-auto">
                  <div className="graphFrame mt-6 min-w-[600px]">
                    {dashboardData.chartData.map((batch, index) => {
                      const height = (batch.yield / maxYield) * 80;

                      return (
                        <div key={index} className="graphBarWrapper">
                          <div
                            className="graphBar"
                            style={{
                              height: `${height}%`,
                          ...
                          >
                            <span className="graphValue">{batch.yield} ml</span>
                          </div>
                          <div className="graphLabel">{batch.batchId}</div>
                        </div>
                      );
                    })}
                  </div>
            ...
            >
              <h2 className="text-2xl font-extrabold bg-gradient-to-r from-yellow-400 via-yellow-200 to-yellow-500 bg-clip-text text-transparent animate-shine">
                ...
                <div className="min-w-[900px]">
                  ...
                  {filteredBatches.map((batch) => (
                    ...
                    >
                      ...
                      <span>{batch.certificateFileName || "N/A"}</span>
                      <span
          ...
          <div className="lg:hidden space-y-4 mt-5">
                {filteredBatches.map((batch) => (
                  <div key={batch._id} className="bg-green-900 rounded-xl p-4">
                    ...
                    <p>
                      <strong>Plant:</strong> {batch.plantVariety}
                    </p>
                    <p>
                    ...
                    <p>
                      <strong>Yield:</strong> {batch.yield} ml
                    </p>
                    <p>
                      <strong>Certificate:</strong>{" "}
                      {batch.certificateFileName || "N/A"}
                    </p>
                    <p>
            ...
            {viewBatch && (
              <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50">
                <div className="bg-white rounded-xl shadow-2xl w-[300px] md:w-[380px] max-w-[90%] p-4 md:p-5">
                  ...
                  <div className="space-y-2 text-black text-sm md:text-base">
                    <p>
                      <strong>Batch ID:</strong> {viewBatch.batchId}
                    </p>
                    <p>
                      <strong>Plant Variety:</strong> {viewBatch.plantVariety}
                    </p>
                    <p>
                    ...
                    <p>
                      ...
                      {new Date(
                        viewBatch.distillationDate
                      ).toLocaleDateString()}
                    </p>
                    <p>
                      <strong>Yield:</strong> {viewBatch.yield} ml
                    </p>
                    <p>
                      <strong>Certificate:</strong>{" "}
                    ...
                    <p>
                      <strong>Status:</strong> {viewBatch.status}
                    </p>
                    <p>
                      <strong>Buyer:</strong>{" "}
